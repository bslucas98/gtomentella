...
###QUEUE job
# TASK=04_alignReads
# SJOBNAME=04_alignReads_04_merge
...
ls -1 ${PROJDIR}/analysis/${TASK}/04_alignReads_03_sort/{1_Leaves,2_Shoots,3_Stems,4_Flowers,5_Pods,6_Roots,7_Senesce,8_Crown}/*.out.bam > list_SortedBAM_gtom_tissues.txt

singularity exec ${PROJDIR}/containers/samtools_1.15.sif samtools merge \
-@ ${NUMCPU} \
--reference ${ASM} \
-o Gtom_allTissues.merged.bam \
--output-fmt BAM \
...
Gtom_allTissues.merged.bam > out_mergeidx.txt 2> err_out_mergeidx.txt
